...
pub fn get_interest_rate<'a>(
    ...
    let mut before_from_rates: Vec<&Rate> = interests.iter().filter(|rate| rate.time < from_time).collect();
    ...
    let rates: Vec<&Rate> = interests.iter().filter(|rate| from_time <= rate.time && rate.time <= to_time).collect();  
    res.extend(rates);
    res
}
