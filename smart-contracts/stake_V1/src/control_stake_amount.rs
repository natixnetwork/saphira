...
/// 
pub fn control_stake_amount(staking_pool: &mut StakePool, address: [u8; 32], amount: Option<u64>) -> ProgramResult {

    msg!("Checking amount {:?}, min: {:?}, max: {:?}", amount, staking_pool.config.min_stake_amount, staking_pool.config.max_stake_amount);

    if amount.is_some() {
        if amount.unwrap() < staking_pool.config.min_stake_amount || amount.unwrap() > staking_pool.config.max_stake_amount {
            return Err(ProgramError::Custom(101));
        }
    }


    let sum_stakes = amount.unwrap_or(0) + give_stakes(staking_pool, address).iter().map(|staker| staker.amount).sum::<u64>();
